...
{
  static const uint32_t freqTable[4] = {48000, 44100, 22050, 11025};

  // Default format (16-bit stereo, 48000 Hz) is tried first and usually succeeds,
  // but we also try other formats before giving up
  for (uint8_t bits_per_sample = 16; bits_per_sample != 0; bits_per_sample -= 8) {
    for (uint8_t n_channels = 2; n_channels != 0; n_channels--) {
      for (uint8_t freq_index = 0; freq_index < 4; freq_index++) {
...
size_t Win32AudioOutput::addSamples(const uint8_t* data, size_t size)
{
  if (haveWO) {
    size_t sample_size = getSampleSize();
  ...
  for (HdrInSlist* hdr = spare_hdrs; hdr != NULL; hdr = hdr->next) {
    ...
    if (hdr->starvedWhenDone && (hdr->streamId == currentStreamId)) {
      ...
      if (now > hdr->starvationTimestamp) {
        ...
        if (extraDelayInMillisec < ((uint32_t)delay_in_millisec))
          extraDelayInMillisec = ((uint32_t)delay_in_millisec);
      }
    }
  }
